...
import { BlockDefinition, BlockParameter, Connection, BlockInstance, BlockPort, AudioContextState, WeightedPrompt, LiveMusicGenerationConfig } from '../types';
import { 
...
import { LiveMusicService, LiveMusicServiceCallbacks, PlaybackState, Scale, MusicGenerationMode, DEFAULT_MUSIC_GENERATION_CONFIG } from '../services/LiveMusicService';
...
export interface AudioEngineHookReturn {
...
export const useAudioEngine = (appLog: (message: string, isSystem?: boolean) => void): AudioEngineHookReturn => {
  ...
  const managedLyriaServiceInstancesRef = useRef<Map<string, ManagedLyriaServiceInfo>>(new Map()); // For LiveMusicService
  ...
  const [isAudioWorkletSystemReady, setIsAudioWorkletSystemReady] = useState(false);
  ...
  const registerWorkletProcessor = useCallback(async (
    ...
    try {
      ...
      setAudioInitializationError(prev => {
        const newError = `RegFail ('${processorName}'): ${error.message.substring(0,100)}`;
        if (prev && prev.includes(newError)) return prev;
  ...
  const removeLyriaServiceForInstance = useCallback((instanceId: string) => {
    ...
    if (info) {
      ...
      try {
        // Disconnect from master gain if applicable
  ...
  const updateAudioGraphConnections = useCallback((
    ...
    connections.forEach(conn => {
      ...
      // Lyria Master does not have audio inputs other than CVs.
  ...
  const setupLyriaServiceForInstance = useCallback(async (
    ...
    const serviceCallbacks: LiveMusicServiceCallbacks = {
      ...
      onSetupComplete: () => addBlockLog("Lyria Service setup complete and ready."),
  ...
  const updateLyriaServiceState = useCallback((
    ...
    if (blockInternalState.reconnectRequest) {
      ...
      service.stop();
      if (blockInternalState.playRequest) { // Handles restart case (stop then play)
    ...
    // Config and prompts updates can happen if session is valid (connected or paused)
    if (service.isConnected() || service.getPlaybackState() === 'paused') {
        if (blockInternalState.configUpdateNeeded) {
            ...
            if (blockInputs.scale_cv_in !== undefined && blockInputs.scale_cv_in !== null) newConfig.scale = blockInputs.scale_cv_in as GenAIScale; // Cast to GenAIScale for Lyria SDK
            else if (blockParams.scale !== undefined) newConfig.scale = blockParams.scale as GenAIScale; // Cast to GenAIScale
            ...
            else if (blockParams.seed === 0) newConfig.seed = undefined; // 0 means auto in UI, so undefined for service
  ...
  }, [audioContext, getLyriaServiceInstance]);
