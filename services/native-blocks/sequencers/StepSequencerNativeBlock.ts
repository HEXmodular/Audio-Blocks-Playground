...
export class StepSequencerNativeBlock implements CreatableNode {
    ...
    private gateSubscription: Tone.Emitter<string> | undefined; // Subscription to the gate emitter for this instance
            ...
            parameters: createParameterDefinitions([
                {
                    id: 'sequence',
                    name: 'Sequence',
                    type: 'step_sequencer_ui', // This type would be handled by StepSequencerControl
    ...
    public getEmitter(outputId: string): Tone.Emitter | undefined {
        if (outputId === 'gate_out') {
            return this._gateEmitter.on('gate_change', (data) => {
                console.log(`[StepSequencerNativeBlock] Gate output changed: ${data.newState}`);
            });
        }
        ...
        if (instance.parameters) {
            ...
            if (newDesignatedEmitter?.on) {
                this.gateSubscription = newDesignatedEmitter.on('gate_change', (payload: { newState: boolean }) => {
                    ...
                    const isActive = this._sequence[this._currentStep]
                    console.log(`[StepSequencerNativeBlock] Gate input changed: ${isActive} at step ${this._currentStep}`, this._sequence);
                    
                    this._gateEmitter.emit('gate_change', { newState: isActive });
                });
            }
    ...
    private updateSequenceFromParams(params: BlockParameter[], internalState?: any): void {
        ...
        if (sequenceParam && Array.isArray(sequenceParam.currentValue)) {
            ...
            // console.log(`[StepSequencerNativeBlock] Sequence updated from parameters:`, this._sequence);
            if (internalState) {
