...
export const useAudioEngine = (
  ...
  const registerWorkletProcessor = useCallback(async (
    ...
    try {
      ...
      if (!actualClassName) {
        ...
        setAudioInitializationError((prevError: string | null) => prevError || `Class name determination failed for ${processorName}`);
        return false;
      }
      ...
      setAudioInitializationError((prevError: string | null) => {
        const newError = `RegFail ('${processorName}'): ${error.message.substring(0,100)}`;
        if (prevError && prevError.includes(newError)) return prevError;
  ...
  }, [appLog, setAudioInitializationError, onStateChangeForReRender]);
  ...
  const toggleGlobalAudio = useCallback(async (): Promise<boolean> => {
    ...
    if (!localAudioContextRef) {
      ...
      setIsAudioWorkletSystemReady(false);
      setAudioInitializationError((prevError: string | null) => prevError || "AudioContext creation/retrieval failed in toggleGlobalAudio.");
    ...
    if (currentContextState === 'suspended') {
      ...
      try {
        ...
        if (currentContextState === 'suspended' || currentContextState === 'closed') {
            setIsAudioGloballyEnabled(false);
            setAudioInitializationError((prevError: string | null) => prevError || "Context remained suspended or closed after resume attempt.");
        ...
        setIsAudioGloballyEnabled(false);
        setAudioInitializationError((prevError: string | null) => prevError || `Resume error: ${(resumeError as Error).message}`);
        return false;
      }
        ...
        if (!localAudioContextRef) { 
             setIsAudioGloballyEnabled(false);
             setAudioInitializationError((prevError: string | null) => prevError || "Re-initialization failed to produce a context.");
        ...
        if (currentContextState !== 'running') {
            setIsAudioGloballyEnabled(false);
            setAudioInitializationError((prevError: string | null) => prevError || "Re-initialization failed to produce running context.");
    ...
    if (isAudioGloballyEnabled) {
      ...
      if (workletsAreReady && localAudioContextRef && localAudioContextRef.state === 'running') { 
          ...
          setIsAudioGloballyEnabled(false);
          setAudioInitializationError((prevError: string | null) => prevError || "Worklets not ready or context not running, cannot enable audio system fully.");
  ...
  }, [isAudioGloballyEnabled, initializeBasicAudioContext, checkAndRegisterPredefinedWorklets, appLog, setIsAudioGloballyEnabled, setIsAudioWorkletSystemReady, setAudioInitializationError, onStateChangeForReRender]);
  ...
  const setupManagedAudioWorkletNode = useCallback(async (
    ...
    if (!registeredWorkletNamesRef.current.has(definition.audioWorkletProcessorName)) {
        ...
        if (!regSuccess) {
            ...
            setAudioInitializationError((prevError: string | null) => prevError || `WorkletNode RegFail: ${definition.audioWorkletProcessorName}`);
            return false;
        }
    ...
    try {
      ...
      setAudioInitializationError((prevError: string | null) => prevError || `WorkletNode Error: ${definition.audioWorkletProcessorName} - ${e.message.substring(0,100)}`);
      return false;
    }
  ...
  const updateLyriaServiceState = useCallback((
    ...
    if (service.isConnected() || service.getPlaybackState() === 'paused') {
        if (blockInternalState.configUpdateNeeded) {
            ...
            if (blockInputs.scale_cv_in !== undefined && blockInputs.scale_cv_in !== null && Object.values(GenAIScale).includes(blockInputs.scale_cv_in as GenAIScale)) {
                newConfig.scale = blockInputs.scale_cv_in as GenAIScale; 
            } else if (blockParams.scale !== undefined && Object.values(GenAIScale).includes(blockParams.scale as GenAIScale)) {
