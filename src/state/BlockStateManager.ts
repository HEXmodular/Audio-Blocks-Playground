...
export class BlockStateManager {
  ...
  private _getLyriaInitialInternalState(definition: BlockDefinition, existingState?: Record<string, any>): Record<string, any> {
    return {
        ...(existingState || {}), // Preserve any existing state if merging
        ...
        configUpdateNeeded: true, // Start with true to send initial params
        promptsUpdateNeeded: true, // Start with true to send initial prompt
        trackMuteUpdateNeeded: true,
        autoPlayInitiated: false, // NEW for auto-play logic
        wasPlayingBeforeGateLow: false, // NEW for gate logic
        ...
        prevStopTrigger: false, // Input trigger states
    ...
    return rawInstances.map((loadedInst: any) => {
      ...
      if (definition) {
        if (definition.id === LYRIA_MASTER_BLOCK_DEFINITION.id) {
            initialInternalState = this._getLyriaInitialInternalState(definition, initialInternalState);
        } else {
            initialInternalState.needsAudioNodeSetup = !!(definition.audioWorkletProcessorName || definition.id.startsWith('native-') || definition.id === 'gain-v1' || definition.id === 'system-audio-output-v1' || definition.id === 'analyser-oscilloscope-v1');
  ...
  public addBlockInstance(definition: BlockDefinition, name?: string, position?: { x: number; y: number }): BlockInstance {
    ...
    let initialInternalState: Record<string, any> = {};
    if (definition.id === LYRIA_MASTER_BLOCK_DEFINITION.id) {
        initialInternalState = this._getLyriaInitialInternalState(definition);
    } else {
        let needsAudioSetup = !!(definition.audioWorkletProcessorName || definition.id.startsWith('native-') || definition.id === 'gain-v1' || definition.id === 'system-audio-output-v1' || definition.id === 'analyser-oscilloscope-v1');
        if (definition.id === RULE_110_BLOCK_DEFINITION.id) { 
            needsAudioSetup = false;
        }
        initialInternalState = { needsAudioNodeSetup: needsAudioSetup };
    }
        ...
        if (definition && (definition.id === RULE_110_BLOCK_DEFINITION.id || definition.id === RULE_110_OSCILLATOR_BLOCK_DEFINITION.id)) {
          ...
          if (newCoreLengthParam && oldCoreLengthParam && newCoreLengthParam.currentValue !== oldCoreLengthParam.currentValue) {
            ...
            if (patternParamInstance && Array.isArray(patternParamInstance.currentValue)) {
              ...
              const maxPatternIndex = (patternParamInstance.steps || 18) -1; 
              ...
              if (idxOldR >= 0 && idxOldR <= maxPatternIndex && idxNewR >= 0 && idxNewR <= maxPatternIndex) {
                if (idxOldR !== idxNewR) {
                  ...
                  if (newCL > oldCL) { 
                    ...
                    modifiedPatternArrayValue[idxNewR] = stateAtOldR; 
                    if (idxOldR < idxNewR) modifiedPatternArrayValue[idxOldR] = false; 
                  } else { 
                     modifiedPatternArrayValue[idxNewR] = stateAtOldR; 
                     for (let i = idxNewR + 1; i < idxOldR +1 && i <= maxPatternIndex; i++) {
                        modifiedPatternArrayValue[i] = modifiedPatternArrayValue[i+1] ?? false; 
                     }
                     if(idxOldR <= maxPatternIndex) modifiedPatternArrayValue[idxOldR] = false; 
                  }
                }
