...
import { BlockDefinition, BlockParameterDefinition, BlockPort, BlockParameter } from './types';
...
export const createParameterDefinitions = (
  ...
  return params.map(pDefProto => {
    ...
    return {
      ...
      steps: pDefProto.steps,
      isFrequency: pDefProto.isFrequency
    };
  });
};
...
BPM_FRACTIONS.sort((a, b) => b.value - a.value);

export const SEQUENCER_BPM_FRACTIONS = BPM_FRACTIONS.filter(f => f.value <=4 && f.value >= 1/32);
