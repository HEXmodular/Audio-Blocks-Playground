...
import { BlockInstance, BlockDefinition, BlockParameter, BlockPort } from '../types';
import { TrashIcon, ExclamationTriangleIcon } from './icons';

const GRID_STEP = 20;
const COMPACT_BLOCK_WIDTH = 120; // Changed from 180 to 120
...
const calculateBlockHeight = (hasParam: boolean, inputPortsCount: number, outputPortsCount: number): number => {
  let height = COMPACT_BLOCK_HEADER_HEIGHT;
  if (hasParam) {
    height += PARAM_DISPLAY_HEIGHT + COMPACT_BLOCK_VERTICAL_PADDING; // Add padding if param shown
  }
  const maxPorts = Math.max(inputPortsCount, outputPortsCount);
  const portsAreaHeight = Math.max(PORT_STUB_AREA_MIN_HEIGHT, maxPorts * (PORT_STUB_DIAMETER + 4) + 8); // Add some padding around ports
  
  // If no param, ports area directly under header. If param, it's separate.
  // This logic needs refinement if param and ports are in same visual plane.
  // For now, assuming param area is distinct if present, and ports are always there.
  // Let's assume the block needs space for ports regardless of param.
  // The height of the block should accommodate the header and the tallest of (param display OR port area).
  // This simplified compact view might not show ports if a param is shown. Let's stick to ports for now.

  // Revised height calculation: Header + (Max of ParamDisplay or PortsArea) + Padding
  // For simplicity, let's assume the main body of the block is for ports primarily.
  // The single param display, if any, will be above the ports or integrated.
  // Let's revert to a simpler height calculation for this iteration of drag-and-drop focus.
  // It will just be header + some body for ports.
  
  // Simpler: Header + fixed body for a few ports.
  // The complex height calculation was from the previous version which tried to fit everything.
  // New focus: Clean block, ports on side for dragging.
  // The provided design for BlockInstanceComponent now only shows name and one numerical param.
  // The ports are just stubs on the side.
  // So the height is: Header + (Optional Param Display) + Padding

  let contentHeight = 0;
  if (hasParam) {
    contentHeight += PARAM_DISPLAY_HEIGHT + COMPACT_BLOCK_VERTICAL_PADDING;
  } else {
    // Even if no param, give some minimal body height for visual balance and future port text (if any)
...
const BlockInstanceComponent: React.FC<BlockInstanceComponentProps> = ({
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    if ((e.target as HTMLElement).closest('.js-interactive-element') || (e.target as HTMLElement).closest('[data-port-id]')) {
      // If click is on delete button or a port stub, don't start block drag.
      // Port stub mousedown is handled separately.
        return;
    }
    e.preventDefault();
    e.stopPropagation();
  ...
  const getPortY = (index: number, count: number, totalBlockHeight: number) => {
    // Distribute ports along the central 70% of the block's height, excluding header
    const usableHeight = totalBlockHeight - COMPACT_BLOCK_HEADER_HEIGHT;
    const marginTop = COMPACT_BLOCK_HEADER_HEIGHT + usableHeight * 0.15; // Start 15% down from header
    const portAreaHeight = usableHeight * 0.70; // Use 70% of body height for ports
  ...
  return (
    <div
      ...
      data-instance-id={blockInstance.instanceId} // For event bubbling checks if needed
    >
      {/* Header */}
      <div 
      ...
      <div className="flex-grow flex flex-col justify-center px-2.5 py-1 relative">
        ...
        {!firstNumericalParamInstance && (
           <div className="h-full" style={{minHeight: `${PARAM_DISPLAY_HEIGHT}px` }}></div> // Placeholder
        )}
      </div>
        ...
        return (
          <div
            ...
            style={{
              ...
              left: `${-PORT_STUB_DIAMETER / 2}px`, // Center on edge
              top: `${portY - PORT_STUB_DIAMETER / 2}px`,
              transform: 'translateX(0%)', // For potential future animations or precise centering
            }}
      ...
      {blockDefinition.outputs.map((port, index) => {
        ...
        return (
          <div
            ...
            style={{
              width: `${PORT_STUB_DIAMETER}px`, height: `${PORT_STUB_DIAMETER}px`,
              right: `${-PORT_STUB_DIAMETER / 2}px`, // Center on edge
              top: `${portY - PORT_STUB_DIAMETER / 2}px`,
